...
$data_lele = readLeleAll();



if(isset($_POST["sbmt-tambah"])){
    
    $pesan = createKolam($_POST);
}

if(isset($_POST["sbmt-hapus"])){
    $id = $_POST['id_kolam'];
    $pesan = deleteKolam($id);
}

if(isset($_POST["sbmt-edit"])){
    $pesan = editKolam($_POST);
}
?>
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Data Kolam</h1>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Mau Nambah Kolam?</h5>
                ...
                <form id="formTambah" action="" method="POST">
                        ...
                        <input type="number" min="0" class="form-control" name="jumlah_lele" id="jumlah_lele" autocomplete="off"  required>
                        </div>
                        <div class="mb-3">
                        <label for="id_lele" class="form-label">Lele</label>
                        <select class="form-select" id="id_lele" name="id_lele" value='' required>
...
<script>
    ...
    $('#btn-refresh').on('click', function () {
        $('#dinamisData').empty();
        $('#dinamisData').load('../../model/ajax/kolam.php')
    })

    $('#btn-asc').on('click', function () {
        $('#dinamisData').load('../../model/ajax/kolam.php?asc=true&desc=false');
    })
    $('#btn-desc').on('click', function () {
        $('#dinamisData').load('../../model/ajax/kolam.php?asc=false&desc=true');
    })

    $('#btn-loadMore').on('click', function () {
        $('#dinamisData').load('../../model/ajax/kolam.php?next='+page)
        page+=3
    }).trigger('click');

    $('#kotakpencarian').on('keydown', function () {
        $('#dinamisData').load('../../model/ajax/kolam.php?keyword=' + $('#kotakpencarian').val());
    });

    $('#kotakFilterJenisKelamin').on('change', function () {
        $('#dinamisData').load('../../model/ajax/kolam.php?jk=true&keyword=' + $('#kotakFilterJenisKelamin').val());
    });
